...
Route::prefix('head')->name('head.')->middleware(['auth.admin', 'role.head'])->group(function () {
    ...
    Route::post('/setup/complete', function (Request $request) {
        $validated = $request->validate([
            'election_name' => 'required|string|min:3|max:255',
            'end_time' => 'nullable|date|after:now',
        ]);

        // Safety: prevent duplicate election creation if one already exists
        if (\App\Models\Election::first()) {
            return redirect()->route('head.dashboard')->with('error', 'An election already exists.');
        }
        ...
        $election = \App\Models\Election::create([
            ...
            'control_number' => '', // Auto-generated by DB trigger
        ]);
